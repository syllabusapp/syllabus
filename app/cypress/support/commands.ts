...
Cypress.Commands.add('login', () => {
  const user = cy;
  user.visit('/');
  user.getByTestId('email-input').type('devan@devanb.us');
  user.getByTestId('password-input').type('secret42');
  user.getByTestId('login-button').click();
  user.url().should('eq', `${Cypress.config().baseUrl}/`);
});
